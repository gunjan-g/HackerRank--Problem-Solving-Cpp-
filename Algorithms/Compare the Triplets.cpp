...
vector<int> compareTriplets(vector<int> a, vector<int> b) {
   ...
   //if element in array a is greater than array b at same index, increment count_a otherwise increment count_b
   for(i=0;i<3;i++){
      if(a[i]>b[i]) count_a++;
      else if(a[i]<b[i]) count_b++;
    }
    
    //push the ans in vector c and return it
    c.push_back(count_a);
    c.push_back(count_b);
